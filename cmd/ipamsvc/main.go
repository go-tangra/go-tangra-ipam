// Command ipamsvc runs the ipam service. `ipamsvc -config <path>` starts the
// service (applying migrations); `ipamsvc bootstrap -config <path>` migrates.
package main
...
func main() {
	if len(os.Args) > 1 && os.Args[1] == "bootstrap" {
		if err := bootstrap(os.Args[2:]); err != nil {
			fmt.Fprintln(os.Stderr, "ipamsvc:", err)
			os.Exit(1)
		}
		return
	}
...
func loadConfig(args []string) (config.Config, error) {
	fs := flag.NewFlagSet("ipamsvc", flag.ContinueOnError)
	...
	for _, w := range cfg.Warnings() {
		fmt.Fprintln(os.Stderr, "ipamsvc: warning:", w)
	}
