...
/**
 * Calls the ipam API through the gateway. Non-2xx responses reject with
 * ApiError carrying the server's closed-vocabulary `reason`. This client is
 * JSON-only; the ipam API takes no multipart uploads.
 */
...
export function describe(err: unknown): string {
  ...
  switch (err.reason) {
    ...
    case 'not_found':
      return 'Not found.'
    case 'conflict':
      return 'That operation is not possible in the current state.'
