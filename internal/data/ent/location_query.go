...
func (_q *LocationQuery) QuerySubnets() *SubnetQuery {
...
func (_q *LocationQuery) QueryVlans() *VlanQuery {
...
func (_q *LocationQuery) QueryDevices() *DeviceQuery {
...
func (_q *LocationQuery) WithSubnets(opts ...func(*SubnetQuery)) *LocationQuery {
	query := (&SubnetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LocationQuery) WithVlans(opts ...func(*VlanQuery)) *LocationQuery {
	query := (&VlanClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LocationQuery) WithDevices(opts ...func(*DeviceQuery)) *LocationQuery {
	query := (&DeviceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(location.FieldCreateBy).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := _q.withSubnets; query != nil {
		if err := _q.loadSubnets(ctx, query, nodes,
			func(n *Location) { n.Edges.Subnets = []*Subnet{} },
			func(n *Location, e *Subnet) { n.Edges.Subnets = append(n.Edges.Subnets, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withVlans; query != nil {
		if err := _q.loadVlans(ctx, query, nodes,
			func(n *Location) { n.Edges.Vlans = []*Vlan{} },
			func(n *Location, e *Vlan) { n.Edges.Vlans = append(n.Edges.Vlans, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDevices; query != nil {
		if err := _q.loadDevices(ctx, query, nodes,
			func(n *Location) { n.Edges.Devices = []*Device{} },
			func(n *Location, e *Device) { n.Edges.Devices = append(n.Edges.Devices, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LocationQuery) loadSubnets(ctx context.Context, query *SubnetQuery, nodes []*Location, init func(*Location), assign func(*Location, *Subnet)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Location)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subnet.FieldLocationID)
	}
	query.Where(predicate.Subnet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.SubnetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *LocationQuery) loadVlans(ctx context.Context, query *VlanQuery, nodes []*Location, init func(*Location), assign func(*Location, *Vlan)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Location)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(vlan.FieldLocationID)
	}
	query.Where(predicate.Vlan(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.VlansColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *LocationQuery) loadDevices(ctx context.Context, query *DeviceQuery, nodes []*Location, init func(*Location), assign func(*Location, *Device)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Location)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(device.FieldLocationID)
	}
	query.Where(predicate.Device(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.DevicesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *LocationQuery) Modify(modifiers ...func(s *sql.Selector)) *LocationSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *LocationSelect) Modify(modifiers ...func(s *sql.Selector)) *LocationSelect {
