...
func (tx *Tx) init() {
	...
	tx.HostGroupMember = NewHostGroupMemberClient(tx.config)
	tx.IpAddress = NewIpAddressClient(tx.config)
	tx.IpGroup = NewIpGroupClient(tx.config)
	tx.IpGroupMember = NewIpGroupMemberClient(tx.config)
	tx.IpScanJob = NewIpScanJobClient(tx.config)
